...
| 2026&#8209;06 | Shape-Shifting Malicious Code in Software Backdoors via Language Models ↗ | attack、LLM steganography、malicious payload、cover-data evasion | AsiaCCS 2026 | [Official](https://doi.org/10.1145/3779208.3807485) · [Paper](https://eisenhofer.me/data/fard-26-shape.pdf) | [Artifact](https://github.com/mlsec-group/animagus)（仅 decoder 与评测数据；攻击 encoder 未公开） | 针对二进制或显式恶意代码作为 software backdoor 载荷容易暴露，论文让 LLM 在 token 约束下把 payload 编码为自然的 README、CONTRIBUTING、Bash 与 Perl cover，并用微型非 LLM decoder 无损恢复；较大 block 下语言困惑度接近自然文本，人工审计对三类 cover 接近随机、现有自动检测器在低误报阈值下也难识别。 |

...
| --- | --- | --- | --- | --- | --- | --- |
| 2026&#8209;08 | SkillBloat: Token Amplification Attacks via Skill Injection in LLM Coding Agents | attack、Agent DoS、malicious skill、token amplification | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.21929) | 暂未公开 | SkillBloat 将恶意 skill 设计成不妨碍正常任务完成、却诱导 Coding Agent 消耗更多 token 的资源攻击；两阶段筛选与全文迭代优化在多种目标配置上实现 5.4184–10.1455 倍放大，说明 Agent DoS 可以通过看似合法的技能指令持续增加经济成本。 |
...
| 2026 | When Efficiency Becomes a Vulnerability: Computational Cost Attacks on WebAgents | attack、cyber misuse、agent DoS、tool loop | ACL 2026 | [Official](https://aclanthology.org/2026.acl-long.1775/) | 暂未公开 | 针对网页注入可诱导 WebAgent 产生冗长推理而放大成本，CostBomb 由 LLM 生成候选并用 RL selector 选择扰动，在多个真实 web benchmark 上显著增加计算消耗却不妨碍任务完成。 |
...
| 时间 | 论文名称 | 关键词 | 会议中稿情况 | 论文链接 | 代码链接 | 一句话总结 |
| --- | --- | --- | --- | --- | --- | --- |
| 2026 | OTora: A Unified Red Teaming Framework for Reasoning-Level Denial-of-Service in LLM Agents | tool、agent DoS、tool loop、resource exhaustion | ICML 2026 Poster | [Official](https://icml.cc/virtual/2026/poster/61771) | 暂未公开 | 针对模型安全策略会被越狱提示或自动化红队绕过的问题，论文提出 OTora 攻击或威胁分析；摘要实验验证该威胁在所列模型、任务或数据集上成立，直接服务于越狱风险测量与红队覆盖。 |

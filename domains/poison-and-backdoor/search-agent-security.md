...
| 2025&#8209;10 | Agentic Reinforcement Learning for Search Misaligns Instruction-Tuning | analysis、search RL、instruction misalignment、representation steering | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2510.17431) | [Code](https://github.com/Yushi-Y/RL-search-safety) | 针对 search RL 只优化答题奖励可能破坏 instruction-tuned safety；论文测量训练前后的 harmful-query behavior 并定位表示空间中的安全方向；结果部分设置的安全性下降最高 68.6%，representation-guided reward 可在不牺牲搜索准确率的情况下恢复对齐。 |
...
| 时间 | 论文名称 | 关键词 | 会议中稿情况 | 论文链接 | 代码链接 | 一句话总结 |
| --- | --- | --- | --- | --- | --- | --- |
| 2026&#8209;08 | SearchAuditor: Auditing and Attributing Failures in Long-Horizon Search Agents | detection、search agent、evidence manipulation、retrieval integrity | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.05212) | 暂未公开 | SearchAuditBench 汇集 1,243 条平均 65.1K token 的失败搜索轨迹并标注关键步骤、根因与修复；SearchAuditor 通过多视角证据裁决将端到端审计通过率从最强基线的 26.6% 提至 32.3%，且修复后可帮助 Agent 恢复。 |
| 2026&#8209;07 | Answer First, Evidence Second? Uncovering Hidden Risks in Well-Structured AI Search Summaries ↗ | analysis、AI search summary、citation consistency、evidence grounding | SIGIR 2026 | [Official](https://doi.org/10.1145/3805712.3809913) | [Code](https://github.com/icip-cas/AISummary) | 针对生成式搜索把流畅答案与引用排版整齐后可能掩盖证据使用失败，论文审计 Google Search 的 14,175 个 MS MARCO 查询；32.31% 摘要含错误、31.08% 存在 citation inconsistency，且 56.16% 的错误在所引来源已经包含支持证据时仍发生。 |
| 2026&#8209;03 | Assessing Web Search Credibility and Response Groundedness in Chat Assistants | analysis、web search assistant、source credibility、citation groundedness | EACL 2026 | [Official](https://aclanthology.org/2026.eacl-long.115/) | 暂未公开 | 针对联网助手可能放大低可信来源；论文在五类 misinformation-prone topic 上分开审计检索 source credibility 与回答 groundedness；结果四个系统的来源选择差异显著，敏感主题上仍会引用不可信页面。 |

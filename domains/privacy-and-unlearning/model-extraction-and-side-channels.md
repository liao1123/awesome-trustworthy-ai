...
| 时间 | 论文名称 | 关键词 | 会议中稿情况 | 论文链接 | 代码链接 | 研究问题 | 核心 idea | 技术 | 结论 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-09 | Inferring Hidden User Models from the Behavior of Personalized LLM Agents ↗ | attack、personalized agent、user-model extraction、black-box probing | 未确认（arXiv） | [arXiv](https://arxiv.org/abs/2609.03815) | 暂未找到公开代码 | 黑盒交互能否从个性化 Agent 行为反推出其隐藏 user model？ | UMPeek 以自适应探针把行为差异转成对偏好、约束和个体状态的可推断信号。 | 设计查询序列，在不同个性化设置下评估属性恢复、行为区分度和查询效率。 | 个性化 Agent 的隐藏 user model 可被行为侧信道部分恢复，配置和长期状态需要最小披露与访问控制。 |

...
3. 用户明确点名或手动精选的论文采用多领域覆盖优先：先确定主领域，再检查所有实质相关领域；只要论文在研究对象、方法、训练机制、评测或安全结论上对某个叶子领域有明确贡献，就应交叉收录，不要求这些贡献彼此完全独立。每个页面分别编写领域化关键词和四项摘要，不能仅因标题或摘要提到相关术语而复制。
...
7. 论文表格的时间统一按 `YYYY-MM` 写入；月份无法确认时只写 `YYYY`。
...
14. 论文表不再使用单一的“一句话总结”列。标准论文表在链接列之后拆为 `研究问题`、`核心 idea`、`技术` 和 `结论` 四列；四项内容都应基于论文摘要或正文填写，保留关键 threat model、方法名、评测设置和主要边界，不能用泛化占位语句替代。相关研究博客、基础资源和 Tool 表仍按各自的列定义记录。

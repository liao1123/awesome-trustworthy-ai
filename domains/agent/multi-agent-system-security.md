...
| --- | --- | --- | --- | --- | --- | --- |
| 2026&#8209;08 | When Context Gets Root: Privilege Escalation in LLM Harnesses | attack、multi-agent delegation、role privilege、context reconstruction | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.27299) | 暂未公开 | 针对主 Agent 向子 Agent 委派时会重新解释消息来源，论文利用 multi-agent mechanism 将原本低权限的 tool content 重建为子 Agent 的 user message 或 system prompt，从而完成 instruction privilege escalation；该链路跨六种 harness 实现全部 13 个 CIA／RCE 目标。 |
| 2026&#8209;08 | HANSARD: A Reference Architecture for Forensic Readiness, Runtime Witnessing, and Graded Attribution in Autonomous Multi-Agent AI Systems | defense、attribution laundering、runtime witnessing、multi-agent accountability | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.22512) | 暂未公开 | 针对多个自主 Agent 可把有害行为分摊到冗余参与者、令任何单体都不满足 but-for causation 的 attribution laundering，HANSARD 在 Agent 外部设置五类证据捕获点并构建因果图重放；补偿集大小和协同残差用于识别群体合谋贡献，而非只相信涉事 Agent 自产日志。 |
| 2026&#8209;08 | Aligned Alone, Misaligned Together: Forecasting Adversarial Capture in LLM Agent Populations | attack、agent population、committed minority、adversarial capture | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.22444) | 暂未公开 | 论文向 LLM monitor 群体注入始终推动固定决定的坚定少数派，发现单体校准良好并不能阻止集体决策被捕获；仅根据无攻击时的群体响应函数即可预报后续偏移，共享推理只能化解弱攻击并延缓强攻击。 |
| 2026&#8209;08 | Adversarial Attacks in Multi-Agent LLM Pipelines: Unveiling Structural Vulnerabilities in Agentic AI Architectures | analysis、pipeline attack、boundary verification、implicit trust | 未确认（arXiv Comments：Accepted at IEEE GLOBECOM 2026） | [arXiv](https://arxiv.org/abs/2608.00718) | 暂未公开 | 针对一个 Agent 接受的 adversarial content 会被后续节点当作可信输入的问题；论文从生产轨迹归纳 content injection、impersonation、plan deviation 与 memory poisoning，并在相同 pipeline 下跨模型验证；结果表明 ASR 主要随 pipeline structure 而非模型能力变化，防御应在 inter-agent boundary 验证内容、身份、意图与状态。 |
...
| 2026&#8209;05 | Hierarchical Attacks for Multi-Modal Multi-Agent Reasoning | attack、multi-agent reasoning、hierarchical manipulation、multimodal system | CVPR 2026 | [Official](https://openaccess.thecvf.com/content/CVPR2026/html/Zhou_Hierarchical_Attacks_for_Multi-Modal_Multi-Agent_Reasoning_CVPR_2026_paper.html) · [arXiv](https://arxiv.org/abs/2605.13213) | 暂未公开 | 针对多智能体系统的错误会沿角色层级传播，作者分别操纵感知、通信和决策节点并组合攻击，说明协作推理会放大局部恶意输入。 |
...
| 2026 | Propagating Unsafe Actions in LLM Controlled Multi-Robot Collaboration via Single Robot Compromise | attack、multi-robot system、LLM planner、compromise propagation | IJCAI-ECAI 2026 | [Accepted](https://2026.ijcai.org/accepted-papers/?ijtrack=main-track) · [Preprint](https://ijcai-preprints.s3.us-west-1.amazonaws.com/2026/3903.pdf) | [Code](https://github.com/TheFatInsect/InfectBot) | 针对 LLM 规划器通过机器人通信协调行动，作者只攻陷一个入口机器人便让不安全指令沿协作链传播，揭示多机器人系统的级联物理风险。 |

## 集体认知偏差与社会风险

| 时间 | 论文名称 | 关键词 | 会议中稿情况 | 论文链接 | 代码链接 | 一句话总结 |
| --- | --- | --- | --- | --- | --- | --- |
| 2026&#8209;01 | When Agents "Misremember" Collectively: Exploring the Mandela Effect in LLM-based Multi-Agent Systems | analysis、collective memory bias、social influence、MANBENCH | ICLR 2026 | [Official](https://iclr.cc/virtual/2026/poster/10006567) · [arXiv](https://arxiv.org/abs/2602.00428) | [Code](https://github.com/bluedream02/Mandela-Effect) | 针对多个 Agent 会否把群体反复强化的错误信息固化为长期记忆；论文以 MANBENCH 比较四类任务、五种交互协议及不同 memory timescale；结果确认 collective Mandela effect，并通过 cognitive anchoring、source scrutiny 与 model alignment 平均降低 74.40%。 |
| 2025&#8209;11 | When AI Agents Collude Online: Financial Fraud Risks by Collaborative LLM Agents on Social Platforms | analysis、financial fraud、agent collusion、social simulation | ICLR 2026 | [Official](https://iclr.cc/virtual/2026/poster/10008753) · [arXiv](https://arxiv.org/abs/2511.06448) | [Code](https://github.com/zheng977/MutiAgent4Fraud) | 针对单 Agent 安全测试无法反映恶意群体在公开传播、私聊劝诱和转账中的协作风险；论文构建覆盖 28 类诈骗的 MultiAgentFraudBench，并分析互动深度、活跃度与协作失败；结果表明合谋会放大欺诈且恶意 Agent 能适应警告、封禁和 monitor 等干预。 |

## 隐私、拓扑与系统结构

| 时间 | 论文名称 | 关键词 | 会议中稿情况 | 论文链接 | 代码链接 | 一句话总结 |
| --- | --- | --- | --- | --- | --- | --- |
| 2026&#8209;04 | CIA: Inferring the Communication Topology from LLM-based Multi-Agent Systems | attack、topology inference、black-box query、intellectual property | ACL 2026 Main | [Official](https://aclanthology.org/2026.acl-long.815/) · [arXiv](https://arxiv.org/abs/2604.12461) | 暂未公开 | 针对优化后的 MAS communication graph 是高价值 IP 且被视为黑盒不可见；论文以 adversarial query 诱导中间语义并通过 global bias disentanglement 与 weak supervision 推断边；结果平均 AUC 0.87、最高 0.99，表明 output 相关性会泄露内部结构。 |
...
| 2025&#8209;12 | Topology Matters: Measuring Memory Leakage in Multi-Agent LLMs | attack、memory leakage、graph topology、PII extraction | Findings of ACL 2026 | [Official](https://aclanthology.org/2026.findings-acl.1980/) · [arXiv](https://arxiv.org/abs/2512.04668) | [Code](https://github.com/llll121/mama-eval) | 针对通信 topology 如何影响 Agent memory 泄漏缺少定量研究；论文以 MAMA 的 Engram-Resonance 两阶段协议比较六类拓扑和多种攻击者位置；结果连接越密、距离越短且目标中心性越高时泄漏越严重，并支持 topology-aware access control。 |

## 检测与防御

| 时间 | 论文名称 | 关键词 | 会议中稿情况 | 论文链接 | 代码链接 | 一句话总结 |
| --- | --- | --- | --- | --- | --- | --- |
| 2026&#8209;08 | AgentFlow: A Flow-Centric Policy Language and Framework for Securing LLM Agent Systems | defense、delegation boundary、authority flow、multi-agent policy | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.22868) | 暂未公开 | AgentFlow 将 Agent 间委派建模为带标签运行时边，显式限制何种 authority 与敏感字段能够跨 delegation boundary 传播；即使每一步委派单看合理，reference monitor 仍依据完整路径和有状态 taint 阻断组合后形成的越权信息流。 |
...
| 2026&#8209;05 | Trustworthy Agent Network: Trust in Agent Networks Must Be Baked In, Not Bolted On | analysis、agent-to-agent security、safe composition、systemic failure | KDD 2026 | [Official](https://doi.org/10.1145/3770855.3818655) · [arXiv](https://arxiv.org/abs/2605.19035) | 暂未公开 | 论文指出单个 agent 的对齐不能自动组合为安全网络，并以组合鲁棒性、语义隔离、可追责性和跨边界可靠性约束 A2A 状态转移。 |
...
| 2026 | Counterfactual Reasoning for Responsibility Attribution in Probabilistic Multi-Agent Systems | analysis、multi-agent system、responsibility attribution、counterfactual reasoning | IJCAI-ECAI 2026 | [Accepted](https://2026.ijcai.org/accepted-papers/?ijtrack=main-track) · [Preprint](https://ijcai-preprints.s3.us-west-1.amazonaws.com/2026/2007.pdf) | 暂未公开 | 针对多智能体结果失败后难分配责任，作者以反事实责任和 Shapley value 建立形式框架，并求解兼顾责任与回报的稳定策略。 |
| 2026 | Responsibility in Multi-Agent Sequential Decision-Making: Comparing Human Judgments to Formal Models of Causal Attribution | analysis、responsibility attribution、human judgment、sequential decision | IJCAI-ECAI 2026 | [Accepted](https://2026.ijcai.org/accepted-papers/?ijtrack=special-track-on-human-centred-ai) · [Preprint](https://ijcai-preprints.s3.us-west-1.amazonaws.com/2026/HC78.pdf) | 暂未公开 | 针对高风险多智能体失败的形式归责可能不符合人类判断，作者以大规模实验比较多种因果定义，发现没有单一方法在全部场景稳定对齐。 |

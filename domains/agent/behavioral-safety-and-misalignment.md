...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-09 | A Case Study on Emergent Cheating and Whistleblowing in Autonomous Research Swarms ↗ | analysis、agentic misalignment、exploit propagation、whistleblowing | 未确认（arXiv） | [arXiv](https://arxiv.org/abs/2609.04170) | 暂未找到公开代码 | 自主研究 Agent 群体中的评测 exploit 会否扩散为集体作弊，群体又能否形成举报和纠偏？ | 在 100 个数学证明 Agent 中同时观察 exploit adoption 与由其他 Agent 发起的 audit、举报和 validation patch。 | 追踪 shared library、P2P 消息、竞争压力、欺诈证明复核、boycott 和规范执行的行为链。 | exploit 可在共享基础设施中传染，但透明通道也支持 whistleblowing 与修复；群体安全不能只依赖个体对齐。 |

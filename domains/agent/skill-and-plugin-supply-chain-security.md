...
| 时间 | 论文名称 | 关键词 | 会议中稿情况 | 论文链接 | 代码链接 | 一句话总结 |
| --- | --- | --- | --- | --- | --- | --- |
| 2026&#8209;08 | Daydreaming: Stealing Hidden Agent Skills through Black-Box Task Interaction | attack、skill extraction、black-box task interaction、functional reconstruction | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.26733) | 暂未公开 | 针对托管方隐藏 skill 文件并过滤直接泄露仍无法保护可执行能力资产的问题，Daydreaming 通过普通黑盒任务、自适应行为探测、shadow Agent 和本地检查重建多文件 skill；即使启用泄露防御，它也以每个 skill 中位数 32 次调用恢复 86.8% 的原始能力，接近 SigLeak 的四倍。 |
| 2026&#8209;08 | EVOMAL: Self-Poisoning in Self-Evolving Coding Agents | attack、malicious skill template、self-propagation、scanner evasion | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.25776) | 暂未公开 | 攻击者只需向共享库植入 skill 而无需调用它；EvoMal 的良性外观 banner 会诱导 Agent 在新 skill 中复制封装 payload，使恶意副本反复回流供应链并规避只检查攻击者提交名称、代码和签名的防线；投毒后 library 的恶意 skill 数量达到植入量的 4.9–9.0 倍。 |
| 2026&#8209;08 | SkillBloat: Token Amplification Attacks via Skill Injection in LLM Coding Agents | attack、skill injection、token amplification、resource abuse | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.21929) | 暂未公开 | 针对第三方 skill 的可信 instruction channel 可被用于传统恶意行为之外的经济资源攻击，SkillBloat 先筛选多类放大机制，再用 LLM 迭代重写完整 skill；在真实 skill benchmark 的多种 Coding Agent 配置上达到 5.4184–10.1455 倍平均最佳 token 放大，揭示 marketplace 准入还需审计成本副作用。 |
...
| 2026&#8209;04 | How Your Credentials Are Leaked by LLM Agent Skills: An Empirical Study | analysis、credential leakage、cross-modal audit、skill marketplace | ASE 2026 | [Official](https://conf.researchr.org/details/ase-2026/ase-2026-research-track/43/How-Your-Credentials-Are-Leaked-by-LLM-Agent-Skills-An-Empirical-Study) · [arXiv](https://arxiv.org/abs/2604.03070) | 暂未公开 | 作者跨配置、自然语言和代码审计 17,022 个 Skill，发现 520 个含 1,708 项凭据泄漏问题，其中 debug log 占 73.5%，且 89.6% 的泄漏凭据可立即利用。 |
...
| 时间 | 论文名称 | 关键词 | 会议中稿情况 | 论文链接 | 代码链接 | 一句话总结 |
| --- | --- | --- | --- | --- | --- | --- |
| 2026&#8209;08 | ClawSentry: A Progressive Multi-Tier Security Monitor for Safeguarding Autonomous LLM Agents | defense、skill admission、progressive review、session anti-bypass | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.21101) | [Code](https://github.com/Elroyper/ClawSentry) | 针对恶意第三方 skill 可在安装后借改写、换工具和跨轮重试绕过一次性 scanner，ClawSentry 先以 FSPR 做首次包审计，再把未决证据交给有界只读审查，并在调用、执行与后果阶段持续监督；SkillInject 上 contextual ASR 从 39.55% 降至 2.61%，五类 Work Agent 的干净 skill aggregate TSR 保持 98.7%。 |
| 2026&#8209;08 | SkillShield: Prompt-Space Security Skills for LLM Coding Agents | defense、security-skill synthesis、prompt budget、tool-loop policy | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.25817) | 暂未公开 | SkillShield 将已知攻击或 Agent 失败离线压缩为可复用 security skill，并比较 all-classes、per-bundle 与 per-class 三种固定 prompt 预算配置；这些 skill 在 session 内持续生效，无需运行时分类或路由，展示了 skill 不只可能携带供应链风险，也可以作为低开销防御 artifact。 |
...
| 2026&#8209;03 | Latent Reuse in Agent Skills: Multi-modal Clone Detection at Ecosystem Scale | analysis、skill provenance、clone detection、supply-chain tracing | ASE 2026 | [Official](https://conf.researchr.org/details/ase-2026/ase-2026-research-track/172/Latent-Reuse-in-Agent-Skills-Multi-modal-Clone-Detection-at-Ecosystem-Scale) · [arXiv](https://arxiv.org/abs/2603.22447) | 暂未公开 | SkillReuse 联合 YAML、自然语言指令与代码检测 106 万对 Skill 克隆，并为 938 个安全相关 Skill 找到单项工件扫描会漏掉的传播关系，为恶意或缺陷扩展的供应链追踪提供 provenance 证据。 |
| 2026&#8209;03 | MalSkills: Detecting Malicious Skills in the Agentic Supply Chain via Neuro-symbolic Reasoning | detection、malicious skill、neuro-symbolic reasoning、agent supply chain | ASE 2026 | [Official](https://conf.researchr.org/details/ase-2026/ase-2026-research-track/45/MalSkills-Detecting-Malicious-Skills-in-the-Agentic-Supply-Chain-via-Neuro-symbolic-) · [arXiv](https://arxiv.org/abs/2603.27204) | [Code](https://zenodo.org/records/19253764) | MalSkills 将配置、自然语言和代码证据组织为依赖图并做神经符号推理，在 15 万余个现实 Skill 的审计中确认 400 个恶意样本，基准 F1 达 93%。 |
...
| 2026&#8209;03 | Context Matters: Repository-Aware Security Analysis of the Agent Skill Ecosystem | analysis、repository context、false positive、abandoned repository | Agent Skills@ACM CAIS 2026 Workshop | [Official](https://www.agentskills-workshop.org/) · [arXiv](https://arxiv.org/abs/2603.16572) | [Code](https://github.com/holzsec/repository-context-agentskills) | 针对仅凭 SKILL.md 会把大量条目标成恶意的问题，论文联合三类分发源、GitHub repository 与行为上下文重新测量；结果显著降低误报，并识别废弃仓库被接管等此前忽略的攻击向量。 |
...
| 时间 | 论文名称 | 关键词 | 会议中稿情况 | 论文链接 | 代码链接 | 一句话总结 |
| --- | --- | --- | --- | --- | --- | --- |
| 2026&#8209;08 | MaliciousSkillBench: A Comprehensive Benchmark for Malicious Agent Skill Detection | benchmark、malicious skill detection、source-disjoint evaluation、benign false positive | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.19901) | [Code](https://github.com/protectskills/MaliciousSkillBench) | 针对恶意 Agent skill 检测数据来源分散、随机切分会高估泛化，MaliciousSkillBench 从 13 个公开来源规范化 9,740 个 skill，并统一 11 类攻击标签评测三种 learned detector 与三种 scanner；随机切分 Macro-F1 为 0.882–0.932，source-disjoint 降至 0.653–0.665，最强模型在留出来源上仍把 62.4% 良性 skill 误报为恶意。 |

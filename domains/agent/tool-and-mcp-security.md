...
| 时间 | 论文名称 | 关键词 | 会议中稿情况 | 论文链接 | 代码链接 | 一句话总结 |
| --- | --- | --- | --- | --- | --- | --- |
| 2026&#8209;08 | ContextLeak: Exfiltrating LLM Agent Context via Malicious Tools ↗ | attack、malicious tool metadata、context exfiltration、RL optimization | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.27800) | 暂未公开 | 针对恶意工具攻击通常只优化“让 Agent 选中工具”却忽略如何把运行时上下文装入参数，ContextLeak 用强化学习自动生成 tool name 与 description，同时诱导选择和 user prompt、执行轨迹、工具列表披露；攻击在 shadow 与 victim context 显著错配时仍有效，并超过适配后的既有恶意工具攻击。 |
| 2026&#8209;08 | EchoCoT: Extracting Hidden Chain-of-Thought from Large Reasoning Models | attack、tool-call replay、hidden-state disclosure、API interaction | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.20055) | [Code](https://github.com/TrustAIRLab/EchoCoT) | 针对 tool-using reasoning API 会在调用间保留并重放隐藏推理状态、而该接口通常不被视为信息披露面，EchoCoT 借 fidelity feedback 迭代恢复完整 CoT；跨开源与专有模型的近逐字抽取表明，tool-call protocol 需要显式限制中间 reasoning state 的可重放性和跨轮泄漏。 |
| 2026&#8209;08 | When Context Gets Root: Privilege Escalation in LLM Harnesses | attack、tool-result provenance、instruction privilege escalation、context reconstruction | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.27299) | 暂未公开 | 针对低权限 tool result 会在 Agent 委派、持久目标或定时任务的 context reconstruction 中被重新包装为高权限指令，论文定义 tool-to-user 与 tool-to-system privilege escalation；六种 Coding Agent harness 上的攻击完成全部 13 个 CIA／RCE 目标，并绕过三种自动权限审查配置。 |
| 2026&#8209;08 | Beyond the Editing Canvas: Evidence Divergence in OOXML-to-LLM Ingestion | analysis、OOXML ingestion、viewer-parser divergence、evidence integrity | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.25880) | 暂未公开 | 论文把文档 extractor 视为 Agent 工具实现的一部分，证明同一个 spec-valid Word／Excel／PowerPoint 文件可以向 Office 用户与 LLM tool 返回不同的 task-relevant evidence；21 类 evidence fork 影响全部 13 个测试 extractor，说明只审核用户可见文档或 tool schema 无法保证实际进入模型 context 的证据完整性。 |
...
| 时间 | 论文名称 | 关键词 | 会议中稿情况 | 论文链接 | 代码链接 | 一句话总结 |
| --- | --- | --- | --- | --- | --- | --- |
| 2026&#8209;08 | ROPE: Routed Origin Policy Enforcement against Indirect Prompt Injection ↗ | defense、origin-guarded parameter、tool authorization、provenance enforcement | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.27496) | [Code](https://github.com/xhOwenMa/ROPE) | 针对 state-changing tool 的序列和参数只能在运行时确定、仅按用户请求预筛会损失效用的问题，ROPE 对敏感参数执行确定性来源检查，只允许不可伪造地追溯到用户、用户指定来源或权威记录的值进入；四种 Agent 上 ASR 为 1.6%–2.6%，保留 82%–100% 干净效用，既有防线失守的长程攻击在其下成功率为零。 |
| 2026&#8209;08 | TraceGrant: A Contract-Governed Security Framework for the Task-Effect Lifecycle of Networked LLM Agents | defense、tool authorization、evidence-bounded authority、effect verification | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.21126) | 暂未公开 | 针对邮件、云盘、日历和交易平台返回的不可信内容可重定向工具或篡改敏感参数，TraceGrant 先从用户请求固定允许的 tool effect，再禁止检索证据创设 Contract 之外的新权限，并以实际调用结果验证任务完成；其对 defense-aware attack、阶段消融和压力设置的评测补充了零成功基准结果的适用边界。 |
| 2026&#8209;08 | When Tool Outputs Become Commands: Separating Action Induction from Runtime Authorization in Tool-Augmented LLM Agents | defense、tool-result injection、action provenance、runtime authorization | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.27146) | 暂未公开 | 针对不可信 tool output 可从提供数据升级为诱导真实副作用的“命令”，SARA 将动作诱导与执行授权分离，以 Action Probe 保留来源、No-History-Promotion 阻止历史洗白，并仅依据用户目标及已授权执行证据批准调用；AgentDojo 与 AgentDyn 四个主要设置中的 ASR 均不超过 0.63%。 |
| 2026&#8209;08 | SPA: Securing Persistent LLM Agents Across Queries with Plan-First Information-Flow Control | defense、tool information flow、dual-lattice IFC、cross-query persistence | 未确认（arXiv Comments：Submitted to USENIX Security 2027） | [arXiv](https://arxiv.org/abs/2608.27234) | 暂未公开 | 针对恶意 tool data 可改变控制流、进入敏感参数并污染后续查询，SPA 先生成完整声明式计划，再以 confidentiality／integrity 双格追踪工具边和持久 artifact；在 `tool_knowledge` 攻击下，其 ASR 在 AgentDojo 为 0、在多查询 AgentDojo-MQ 为 0.2%。 |
| 2026&#8209;08 | AgentFlow: A Flow-Centric Policy Language and Framework for Securing LLM Agent Systems | defense、tool data flow、sensitive field、runtime mediation | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.22868) | 暂未公开 | 针对不可信内容诱导 Agent 把私有字段交给错误工具或外部 sink，AgentFlow 在工具调用边上携带数据标签和释放状态，限制每个接收方可见字段并由 runtime monitor 强制执行；防护依据端到端信息流而非只扫描单次 tool call。 |
| 2026&#8209;08 | Forgotten in Weights, Recovered by Tools: Agentic Tool Unlearning for LLM Agents | defense、tool-mediated recovery、target-seeking tool call、knowledge leakage | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.21544) | [Code](https://github.com/BaichengDanny/ATU) | 针对 Agent 权重已遗忘目标知识后仍可主动调用搜索、检索和数据库工具恢复它，ATU 在参数 unlearning 之后用轨迹级 RL 同时约束寻找目标的 tool behavior 与 final-answer leakage，并保留对非目标知识的正常工具使用。 |
| 2026&#8209;08 | SecOPD: Mitigating Adaptive Prompt Injections by On-Policy Distillation | defense、indirect prompt injection、tool-call security、adaptive attack | 未确认（arXiv Comments：EMNLP 2026 main conference） | [arXiv](https://arxiv.org/abs/2608.21500) | [Code](https://github.com/pppyb/SecOPD) | 针对工具 observation 中的注入会把 Agent 轨迹引向攻击者任务，SecOPD 以 token-level on-policy distillation 内化 instruction-data 边界；即使工具调用领域完全未出现在训练中，其 ASR 仍为 4.7%，略优于 Meta-SecAlign 的 5.5%。 |
| 2026&#8209;07 | Teaching Small Models When Not to Call Functions: Structured Reasoning for Reducing Tool-Use Hallucinations | defense、function calling、tool hallucination、structured abstention | SIGIR 2026 | [Official](https://doi.org/10.1145/3805712.3809979) | 暂未公开 | 针对小模型会在请求无关或依据不足时仍调用函数；论文用 structured key-value reasoning 训练 tool-use abstention；结果减少无依据执行，同时保留正常 function-calling utility。 |
...
| 2025&#8209;03 | Defeating Prompt Injections by Design ↗ | defense、tool-result isolation、capability policy、secure interpreter | IEEE SaTML 2026 | [Official](https://satml.org/2026/accepted-papers/) · [arXiv](https://arxiv.org/abs/2503.18813) | [Code](https://github.com/google-research/camel-prompt-injection) | 针对 tool output 中的攻击指令会与事实一起进入 planner 并诱导未授权调用，CaMeL 保留数据来源、禁止不可信值改变程序控制流，并以 capability policy 约束其能到达的工具与敏感 sink；这把工具安全从文本识别问题转成可执行的信息流边界。 |

## Benchmark

| 时间 | 论文名称 | 关键词 | 会议中稿情况 | 论文链接 | 代码链接 | 一句话总结 |
| --- | --- | --- | --- | --- | --- | --- |
| 2026&#8209;01 | ToolSafe: Enhancing Tool Invocation Safety of LLM-based Agents via Proactive Step-level Guardrail and Feedback | defense、tool invocation、step-level guard、TS-Flow | Findings of ACL 2026 | [Official](https://aclanthology.org/2026.findings-acl.1850/) · [arXiv](https://arxiv.org/abs/2601.10156) | [Code](https://github.com/MurrayTom/ToolSafe) | 针对不安全 tool call 需要在执行前发现的问题，论文构建 TS-Bench，以多任务 RL 训练 TS-Guard，并用 guard feedback 驱动 TS-Flow；结果平均减少 65% 有害调用并改善受注入时的良性任务完成率。 |
| 2025&#8209;12 | MCP-SafetyBench: A Benchmark for Safety Evaluation of Large Language Models with Real-World MCP Servers | benchmark、MCP server、multi-turn evaluation、cross-server attack | ICLR 2026 | [Official](https://iclr.cc/virtual/2026/poster/10011290) · [arXiv](https://arxiv.org/abs/2512.15163) | [Code](https://github.com/xjzzzzzzzz/MCPSafety) | 针对现有 MCP 测试缺少真实 server 与跨服务协调的问题，论文在五个领域构建覆盖 server、host、user 三侧的多轮攻击；结果所有受测模型都存在明显 vulnerability 与 safety-utility trade-off。 |
| 2025&#8209;10 | MCP Security Bench (MSB): Benchmarking Attacks Against Model Context Protocol in LLM Agents | benchmark、MCP pipeline、real tool execution、NRP | ICLR 2026 | [Official](https://iclr.cc/virtual/2026/poster/10007929) · [arXiv](https://arxiv.org/abs/2510.15994) | [Code](https://github.com/dongsenzhang/MSB) | 针对 MCP 安全结论来自零散 demo、难以横向比较的问题，论文沿 planning、invocation 和 response handling 定义十二类攻击并运行真实工具；结果更强的工具调用和指令跟随能力反而常带来更高 vulnerability。 |
| 2025&#8209;09 | SafeToolBench: Pioneering a Prospective Benchmark to Evaluating Tool Utilization Safety in LLMs | benchmark、prospective safety、malicious instruction、tool risk | Findings of EMNLP 2025 | [Official](https://aclanthology.org/2025.findings-emnlp.958/) · [arXiv](https://arxiv.org/abs/2509.07315) | [Code](https://github.com/BITHLP/SafeToolBench) | 针对在工具执行后才判断风险无法阻止不可逆伤害；论文从 user instruction、tool 本身及二者组合构建 prospective safety benchmark，并提出 SafeInstructTool；结果显示现有方法无法覆盖全部调用风险，而联合视角能提升执行前风险意识。 |
...
| 时间 | 论文名称 | 关键词 | 会议中稿情况 | 论文链接 | 代码链接 | 一句话总结 |
| --- | --- | --- | --- | --- | --- | --- |
| 2026&#8209;08 | The Guard That Cried Wolf: How Scary Words Make Agent Guardrails Refuse Legitimate Actions | benchmark、tool-action authorization、over-refusal、surface-name bias | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.27009) | 暂未公开 | 针对执行前 guard 会把“看起来危险”的 tool action 与真正未授权动作混淆，Cautious Bench 在授权策略不变时仅替换对象名，并以机械标签验证应当放行的 action；六个受测 guardrail 均出现显著误拒上升，说明工具权限判断必须读取 authorization context，而不能从资源名称猜测风险。 |

...
> 开放网页证据被 Search Agent 读取后造成的 endorsement、harmful search 与 research-trajectory hijacking 见 [Search Agent Security](search-agent-security.md)；固定知识库和 GraphRAG 的语料污染见 [RAG Poisoning](../poison-and-backdoor/rag-poison.md)；通用 indirect prompt injection 见 [Prompt Injection](../misc/prompt-injection.md)。

...
- **Policy-level robustness：** SafeVLA、SafeBranch、STRONG-VLA 和 structure-aware fine-tuning 分别用 constrained learning、branch-pair preference 与 robust training 直接约束 policy，使安全不完全依赖推理时附加模块；其跨 embodiment 和 adaptive attack 鲁棒性仍是主要边界。

## Safety Benchmark、Dataset 与 Formal Evaluation

| 时间 | 论文名称 | 关键词 | 会议中稿情况 | 论文链接 | 代码链接 | 一句话总结 |
| --- | --- | --- | --- | --- | --- | --- |
| 2026&#8209;08 | LIBERO-VIFO: Benchmarking the Capability and Safety of Visual Cue Following in Vision-Language-Action Models | benchmark、VLM safety、VLA safety、physical hazard | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.17600) | 暂未公开 | LIBERO-VIFO 以八类视觉 cue 和四个 protocol 分开测试授权跟随与未授权跟随；七个 VLA 即使没有语言指令也会执行 cue 指示任务，暴露视觉控制劫持风险，并在安全关键及真实机器人设置中复现。 |
| 2026&#8209;08 | MANIGUARD: A Benchmark and Data Suite for Specification-Grounded Safety Evaluation and Improvement of Robotic Manipulation | benchmark、VLA safety、physical hazard、trajectory evaluation | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.17386) | 暂未公开 | ManiGuard 用独立于任务成功的 LTLf automaton 对 200 个基础任务、1,000 个场景逐步检查物理安全，并发布 8,000 条标注轨迹；23,000 余 rollout 中 6%–21% 的成功任务仍违规，安全微调虽改善表现但分布偏移下仍有 21%–42% engaged rollout 不安全。 |
| 2026&#8209;08 | FabriMAE I Trust Myself? Self-Evaluating VLA Action Generation with Markov Attention Entropy | benchmark、adversarial robustness、uncertainty calibration、VLM safety | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.16697) | 暂未公开 | 视觉-语言-动作模型 (VLA) 将视觉感知、语言指令和动作生成集成到跨异构架构的端到端策略中；在这项工作中，我们观察到内部视觉模态熵在跨异构 VLA 的成功任务和失败任务之间表现出一致的区别；我们进一步实例化 FabriMAE，以实现无需验证者的测试时操作选择，表明 MAE 引导的多重采样提高了 LIBERO-Plus 上 PI 系列的稳健性，并且观察到的运行时间开销很小。 |
...
| 时间 | 论文名称 | 关键词 | 会议中稿情况 | 论文链接 | 代码链接 | 一句话总结 |
| --- | --- | --- | --- | --- | --- | --- |
| 2026&#8209;07 | Towards General Language-Conditioned Latent Safety Filters | defense、VLM safety、VLA safety、physical hazard | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.00315) | 暂未公开 | 论文让 Hamilton–Jacobi safety actor/critic 接受自然语言约束，从而为不同用户、环境和任务动态指定 VLA safety filter；在抓取、擦桌和堆块任务中减少约束违规，并对同一约束族的未见实例表现出部分迁移。 |

## 检测、审计与取证

| 时间 | 论文名称 | 关键词 | 会议中稿情况 | 论文链接 | 代码链接 | 一句话总结 |
| --- | --- | --- | --- | --- | --- | --- |
| 2026 | RoboFailRing: Retrieval-Augmented and Language Grounding Failure Detection for VLM-enabled Robotic Manipulation | detection、VLM safety、VLA safety、physical hazard | ACL 2026 | [Official](https://aclanthology.org/2026.acl-long.602/) | [Code](https://github.com/DynamicPoet/RoboFailRing) | RoboFailRing 从失败记忆实时检索并把 grounded report 交给 VLM，在 6,000 余条、81 项操纵任务上 OOD 检测成功率达 80%、耗时约减半，真实机器人故因推理平均提升 35%。 |

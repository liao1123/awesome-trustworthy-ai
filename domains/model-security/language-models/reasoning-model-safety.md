...
| --- | --- | --- | --- | --- | --- | --- |
| 2026&#8209;08 | Amplified Does Not Mean Predictive: Reasoning Behaviors in Thinking Models | analysis、VLM safety、reasoning model、safety degradation | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.13760) | [Code](https://github.com/neulab/behavioral-lift) | 哪些推理行为与推理模型中的正确答案相关，面向推理的训练是否会放大这些行为？这种区别很重要，因为面向推理的训练可以使痕迹看起来更加深思熟虑，而不会放大与模型正确性最相关的行为；我们用行为提升来量化这种不匹配，行为提升是一种衡量模型推理轨迹中存在行为与不存在行为时正确性变化程度的指标；我们发现，以推理为导向的训练不会优先放大最高升力行为，从而激发过程级目标，奖励经过校准和扎根的推理，而不是仅仅奖励表面形式。 |

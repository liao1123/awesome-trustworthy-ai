...
| 时间 | 论文名称 | 关键词 | 会议中稿情况 | 论文链接 | 代码链接 | 一句话总结 |
| --- | --- | --- | --- | --- | --- | --- |
| 2026&#8209;07 | Diagnosing and Mitigating Perception-Decision Misalignment in Omni-LLMs via Modality Subspace Activation | defense、omni-modal model、cross-modal risk、modality interaction | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.14655) | 暂未公开 | Omni-Large Language Models (Omni-LLM) 在世界动作模型和自主代理等应用中支持复杂的多模态推理；为了诊断这一点，我们将因果模态敏感性 (CMS) 形式化，通过双镜头框架进行操作：宏观行为层面的答案保留率 (ARR) 和跟踪微观分布变化的 Logit 角度差异 (LAD)；基准测试显示，流行的 Omni-LLM 表现出极低的 CMS，即使删除关键模式，分布变化也可以忽略不计。 |

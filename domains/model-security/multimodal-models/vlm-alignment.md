...
| 时间 | 论文名称 | 关键词 | 会议中稿情况 | 论文链接 | 代码链接 | 一句话总结 |
| --- | --- | --- | --- | --- | --- | --- |
| 2026&#8209;08 | PEA-DPO: Perception-Enhanced Alignment Direct Preference Optimization for MLLMs Alignment | analysis、VLM safety、multimodal alignment、cross-modal generalization | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.19598) | 暂未公开 | 直接偏好优化（DPO）已成为使大语言模型（LLM）与人类偏好对齐的一种有效方法；通过表征分析，我们识别出多模态偏好优化中的一个关键局限，并将其称为视觉不敏感性：模型往往无法区分原始图像与移除了关键视觉上下文的图像；在三个幻觉基准上使用不同规模的多模态大语言模型进行的评估显示，PEA-DPO 能够有效缓解视觉不敏感性，实现更强的多模态对齐，并大幅减少幻觉。 |
| 2026&#8209;08 | When Safety Overrides Vision: Exploring Dynamics between Vision Influence and Safety Alignment in Vision-Language Models | analysis、safety alignment、VLM safety、multimodal alignment | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.18628) | 暂未公开 | 经过对齐的视觉—语言模型（VLM）旨在平衡有视觉依据的推理与安全生成行为；然而，我们观察到一个显著现象：在安全约束指令下，即使接收到完全相同的图像—问题输入，模型也经常拒绝回答那些在默认指令下能够正确回答的问题；这些发现共同揭示了对齐 VLM 中一个此前研究不足的失效模式：即使感知证据仍在内部保留，安全对齐也可能压过基于视觉证据的表达。 |
...
| 时间 | 论文名称 | 关键词 | 会议中稿情况 | 论文链接 | 代码链接 | 一句话总结 |
| --- | --- | --- | --- | --- | --- | --- |
| 2026&#8209;08 | ReWEIGH the Evidence: Calibrating Token-Level Ordinal Visual Evidence to Mitigate Hallucinations in Large Vision-Language Models | defense、VLM safety、multimodal alignment、cross-modal generalization | 未注明（arXiv） | [arXiv](https://arxiv.org/abs/2608.19075) | 暂未公开 | 大型视觉—语言模型（LVLM）经常产生幻觉，生成输入图像并不支持的内容；我们提出 ReWEIGH，一种无需训练的解码干预：它聚合不同视觉位置上的排名，并将每个候选与从无标签图像估计的 token 专用参考值比较。 |

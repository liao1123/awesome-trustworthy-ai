...
| 检查范围 | 主会 First Cycle 与 Second Cycle 录用论文；First Cycle 数据截至 2026-08-22，Second Cycle 数据截至 2026-09-26 |
...
- 官方论文总数：187（First Cycle，截至 2026-08-22）+ 约 240（Second Cycle，截至 2026-09-26，官网列表可能仍在补充）
- 初筛候选：53（First Cycle）+ 约 45（Second Cycle）
- 最终收录：31（First Cycle 完整卡）+ 8（Second Cycle 已在此前日报收录，见文末 Second Cycle 节的 domains 映射）
...
## Second Cycle 整理（2026-09-26）

官网 2026-09 公布 Second Cycle 录用列表（与 First Cycle 同页）。AI/ML 安全相关约 45 篇，其中 8 篇已在此前日报收录进 `domains/`（venue 已补标 `🏷 ACM CCS 2026`）；其余多数尚无公开摘要或 arXiv 版，按本文件口径列入待核验，arXiv 出来后经日报管线收录。

### 已在库的 8 篇（domains 映射）

| 论文 | arXiv | domains 叶子 |
| --- | --- | --- |
| SkillScope: Fine-Grained Least-Privilege Enforcement for Agent Skills | 2605.05868 | poison-and-backdoor/agent-skill-poison-and-backdoor |
| Adaptive Diffusion Freezing: Privacy-preserving Diffusion Models Against MIA | 2609.10608 | privacy-and-unlearning/leakage-audit-and-mitigation |
| Multimodal Resource-Exhaustion Attacks on VLMs via Joint Pixel-Prompt Optimization | 2609.05889 | dos/multimodal-and-embodied-model-dos |
| InceptionRAG: Stealthy Poisoning Attack Against RAG | 2609.16818 | poison-and-backdoor/rag-poison |
| MemPoison: Bypassing Selective Memory Mechanisms to Plant Backdoors in LLM Agents | 2607.14651 | poison-and-backdoor/agent-memory-poison |
| PriMobiBench: Visual Privacy Leakage in VLM-Driven Mobile GUI Agents | 2609.13873 | agent/web-and-computer-use-agent-security |
| Real Money, Fake Models: Deceptive Model Claims in Shadow APIs | 2603.01919 | misc/model-copyright-protection |
| When Scammers Talk Back 类语音攻击面语料见 0925 日报（SR-Fraud/语料相关） | — | — |

（注：Second Cycle 的 SEAL 为 VU Amsterdam 的 MoE 共享专家对齐工作，与库内 ICLR 2025 的 SEAL 同名不同文，未在库。）

### 待核验清单（按第一轮分类，标题依官方列表）

**越狱、安全对齐与提示防护**：LocalAlign（近目标对抗样本对齐训练的可泛化注入防御）、SymSan（参数空间对称性击败 LLM 恶意软件嵌入）、Jailbreaking via Activation Dilution（神经元引导自适应分解越狱）、Thinking Dangerously（推理训练稀释安全）、SEAL-MoE（共享专家对齐强化全局安全）、Robustness of VLMs Against Split-Image Harmful Input、A Large-Scale Measurement of In-Page Prompt Injections Against LLM Web Agents

**智能体、RAG 与应用安全**：zkAgent（单次 transcript 证明的可验证 agent 执行）、MAGE（shadow memory 防 LLM agent 长程威胁）、Comment and Control（上下文接地进化劫持 agentic 工作流）、MetaMancer（工具选择攻击）、ContractGuard（MCP 工具语义合同审计）、Governing AI Agent with Capability and Delegation Chain、PrivEscalate（LLM 自动化 Linux 提权）、Tab Tab Bug（AI IDE 下一编辑建议的安全陷阱）、Talking to the Model（LLM 集成 Android 应用风险）、Understanding the Security Boundary of Obfuscation-based On-Device LLM Protection、CircumVolve（LLM 发现审查规避策略）、IoTProber（RAG 设备识别）、Global-View Reasoning / Uncovering Hidden Attack Surfaces（LLM 漏洞检测与黑盒扫描）、The Deception Delta（LLM 合约取证对抗评测）、IntraGuard（评审外包给商业 chatbot 的委员会侧防御）、LDBF（LLM 驱动 eBPF 探索）、Verifying Consensus Protocols from LLM-assisted TLA+

**投毒、后门与 AI 供应链**：ArchProof（ONNX 休眠门路径后门的输出贡献证书）、Finding FAULTs in Architectural Backdoors（真实条件下触发检测为何失效）、(A)iSpy（ML 基础设施寄生木马）、Detecting Data Poisoning in Code Generation LLMs（黑盒漏洞导向扫描）、Your Synthetic Image Is a Confession（木马化 I2I 模型重建私有输入）、SkillCODER（agent 技能版权审计与归因）、Code-SeC（解码时探测代码 LLM 秘密记忆）、Logit Laundering（规避 LLM 数据使用审计）

**隐私、机密计算与可验证推理**：From Snapshot to Snooping（大型 VLM 地理位置泄漏实证）、When Topology Betrays Privacy（FL 安全聚合的格重建攻击）、ROSETTA（CKKS/TFHE 混合隐私 LLM 解码）、zkTransformer（LLM 推理 ZK 证明）、TWIST（隐私租户专用 LLM 服务）、MOSAIC/TENNOR/Tricycle/PackCNN/DeepIvy/Mitigating Ciphertext Side-Channel（隐私保护 NN 计算族）、Rethinking the Security of DP-SGD、G-Fence（联邦图 DP GNN 训练）

**数据溯源、模型权属与内容真实性**：DWBench（数据集版权水印整体评测）、NotaryMark（T2I 扩散模型黑盒数据集使用审计多比特水印）、PURA（可证无偏鲁棒多比特文本水印）、Proof-of-Authorship for Diffusion-based AIGC、PersonaHate（人格化仇恨数据合成管线）、Does AI Amplify Online Hate?（4chan /pol/ 487M 帖多模态测量）、It Takes a Forum（在线合成 NCII 生态）、Identifying AI Web Scrapers Using Canary Tokens、Tsuikami（语音转换下跨信道源说话人追踪）

**对抗鲁棒性、生物识别与自动驾驶**：Investigating Planning Attacks against End-to-End Autonomous Driving、From Stealthy Data Fabrication to Unsafe Driving（协同感知现实场景攻击）、PhantomTac（标签劫持机器人触觉系统）、Single Medium Multiple Perspectives（AI 服务媒体预处理语义鸿沟）、As Seasons Change（时序特征恶意软件分类）、When Tracking Learns（学习型浏览器指纹）、TDDM-Melatt（解耦记忆扩散加密流量分类）

**HCI/生态测量（扩展）**：GenAI Voice Cloning Scams、Instagram 性威胁感知、LLM 辅助研究工作流隐私认知、AI Tutors 网络安全教育、Learning Less While Asking More（LLM 辅助漏洞学习）

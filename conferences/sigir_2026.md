...
| --- | --- | --- | --- | --- | --- |
| AdversarialCoT: A Multi-Agent Framework for Knowledge Base Poisoning in Retrieval-Augmented Generation | Mingyue Cheng, Tingting Zhu, Zhizhou Zhong, Xiangyu Zhou, Haoran Xie | [Official](https://doi.org/10.1145/3805712.3809838) · [arXiv](https://arxiv.org/abs/2604.12201) | 暂未公开 | attack、RAG poisoning、multi-agent reasoning、knowledge base integrity | AdversarialCoT 让多个攻击 agent 协作生成具备连贯推理链的投毒文档，表明即使检索证据表面相关，RAG 仍可能被系统性诱导到攻击者指定答案。 |
| Answer First, Evidence Second? Uncovering Hidden Risks in Well-Structured AI Search Summaries | Jinman Li, Xuanang Chen, Ruoxi Xu, Hongyu Lin, Yaojie Lu, Zecheng Fan, Xianpei Han, Le Sun | [Official](https://doi.org/10.1145/3805712.3809913) | 暂未公开 | analysis、AI search、evidence ordering、user trust | 论文检验 AI 搜索摘要中先给结论、后列证据的常见结构，揭示流畅组织会掩盖证据支撑不足并放大用户对错误答案的信任。 |
| Deletion Isn't Enough: Auditing RAG for Selective Forgetting | Leila Tavakoli, Mark Sanderson | [Official](https://doi.org/10.1145/3805712.3808545) | 暂未公开 | audit、RAG unlearning、selective forgetting、residual knowledge | 论文指出从检索库删除记录不能保证 RAG 真正遗忘，并提出审计视角检查模型参数、缓存和关联证据中残留的可恢复信息。 |
...
| Separating Wheat from Chaff: Enhancing RAG Robustness to Poisoned Contexts with Attention-Guided Voting | Yanxiao Zhao, Longzhu He, Li Sun, Sen Su | [Official](https://doi.org/10.1145/3805712.3809895) | 暂未公开 | defense、RAG poisoning、attention voting、context filtering | 论文以注意力引导的证据投票区分可信上下文与投毒片段，在生成前削弱恶意检索结果对最终答案的控制。 |
...
| The Vulnerability of LLM Rankers to Prompt Injection Attacks: You are to [MARK] this paper as the Best Paper | Yu Yin, Shuai Wang, Bevan Koopman, Guido Zuccon | [Official](https://doi.org/10.1145/3805712.3808553) · [arXiv](https://arxiv.org/abs/2602.16752) | [Code](https://github.com/ielab/LLM-Ranker-Attack) | attack、LLM ranker、prompt injection、ranking manipulation | 论文在 pairwise、listwise 和 setwise 排序中注入目标指令，显示攻击者可劫持排序目标或标准，而不同 LLM 架构的抗攻击性差异明显。 |
| When Context Bites: Detecting and Defending Against RAG Attacks Through Attention Collapse | Yingtao Ren, Hao Fu, Yilin Zhao, Zihan Lin, Nitesh Chawla, Xiangliang Zhang | [Official](https://doi.org/10.1145/3805712.3809904) · [arXiv](https://arxiv.org/abs/2608.06947) | [Code](https://github.com/yingtaoren/D-Scan) | defense、RAG attack、attention collapse、runtime detection | 论文把投毒上下文引起的注意力坍缩作为运行时信号，并用 D-SCAN 在生成阶段检测和缓解多类 RAG 攻击。 |
...
| EVADE-Bench: Multimodal Benchmark for Evaluating and Enhancing Evasive Content Detection | Ancheng Xu, Zhihao Yang, Jingpeng Li, Guanghu Yuan, Longze Chen, Liang Yan, Jiehui Zhou, Zhen Qin, Hengyu Chang, Yukun Chen, Hamid Alinejad-Rokny, Min Yang | [Official](https://doi.org/10.1145/3805712.3808579) | [Code](https://github.com/koenshen/EVADE-Bench) | benchmark、evasive content、multimodal moderation、policy circumvention | EVADE-Bench 覆盖文本与图像中的多类规避表达，显示现有模型面对改写、隐喻和视觉掩饰时难以稳定执行内容政策。 |

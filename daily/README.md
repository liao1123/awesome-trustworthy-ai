...
- **默认每周一次**：用户说「收集本周」时，用 arXiv 月度归档页（`https://arxiv.org/list/<cat>/2026-MM`，注意是 YYYY-MM 格式）一次性抓整周、按天写文件；已存在的天跳过，缺失的天回补（除非用户说不补）。
- **单日收集**：用户点名某天时用当日官方 `new` 页面。
- 每天 `20:00 US Eastern` 后 arXiv 公告才完整；同一天多次收集时注意去重。

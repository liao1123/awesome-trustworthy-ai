...
### 26. TH-GNN: Heterogeneous Temporal Graph Neural Networks for LLM-Agent Shilling Attack Detection

**内容概述：** 针对 LLM Agent 可同时生成逼真评论、评分和协同行为而绕过单一文本或图检测器，TH-GNN 联合异构时序图、跨模态 attention 与到达间隔 GRU；五类攻击和四个数据集上的总体平均 F1 为 0.870，对 Agent4SR 比最强文本基线提高 10.9 个百分点。

- 收录理由：该工作检测 LLM 驱动的推荐系统刷单攻击，直接属于 AI 安全中的对抗与滥用防护。
- 论文/arXiv：[TH-GNN: Heterogeneous Temporal Graph Neural Networks for LLM-Agent Shilling Attack Detection](https://arxiv.org/abs/2608.20376)
- arXiv ID：`2608.20376`
- 代码：暂未找到公开代码
...
### 29. aiXamine: Unified Black-Box Evaluation of Cross-Dimensional Trade-offs in LLM Safety, Security, and Privacy
...
### 30. The Claws in Plain Sight: Unauthorized Context Disclosure through LLM Agent Tool Calls
...
### 31. Uncovering and Understanding Hidden Dependencies in the LLM API Reseller Ecosystem via Prefix-Cache Side Channels
...
### 32. TraceGrant: A Contract-Governed Security Framework for the Task-Effect Lifecycle of Networked LLM Agents
...
### 33. Ghost Echoes: Semantic Erasure Failure in Retrieval-Backed Applications
...
### 36. Trojaning the Alignment: Stealthy Backdoor Attacks against Graph Foundation Models

**内容概述：** 针对 graph foundation model 的图—文本对齐会抑制单模态 trigger、同时让异常节点或子图易被检查，STAG 协同图触发器与文本 soft prompt 将两种表示推向同一目标类别，并以可读节点和结构正则保持隐蔽；多个 TAG 数据集和代表性 GFM 上均验证了攻击有效性。

- 收录理由：该工作提出针对图基础模型的隐蔽后门攻击，直接涉及 AI 安全中的攻击面分析与防御需求。
- 论文/arXiv：[Trojaning the Alignment: Stealthy Backdoor Attacks against Graph Foundation Models](https://arxiv.org/abs/2608.20991)
- arXiv ID：`2608.20991`
- 代码：[Code](https://github.com/ventr1c/STAG)
- arXiv 分类：`cs.LG`
- 提交日期：2026-08-21
- arXiv Comments：Accepted by ICDM 2026
- 作者及单位：
  - Minhua Lin — The Pennsylvania State University
  - Zhicheng Gao — Independent Researcher
  - Yilong Wang — The Pennsylvania State University
  - Hanqing Lu — Amazon
...
- 中文摘要：文本属性图（TAGs）上的图基础模型（GFMs）将图表示与语言语义对齐，以支持可迁移的图学习。 尽管具有这些优势，但 GFMs 在 TAGs 上的后门脆弱性仍然理解不足，尤其是在图-语言对齐条件下，其中图和文本表示被训练为在共享语义空间中相互约束。 现有后门攻击主要针对图侧或文本侧中的一方，将这两种模态独立对待。 这使得直接适配无效：仅图触发器可能被干净文本语义约束，而仅文本触发器虽改变语言视角，却不会直接移动被对齐和评分的图表示。 TAGs 还带来了隐蔽性挑战，因为触发器会同时暴露为节点文本和局部图结构，使得不连贯的触发器属性或异常子图容易被检查或过滤。 在本文中，我们提出 STAG，这是一个为 TAGs 上 GFMs 的图-语言对齐接口设计的隐蔽木马攻击框架。 STAG 协同一个图触发器生成器与一个文本侧软提示，使附着触发器的图表示和被触发的文本表示朝着同一个目标类别文本区域移动。 为解决 TAG 特有的隐蔽性问题，STAG 通过候选检索将触发器节点实现为可读文本，并对附着触发器的子图施加正则化，使其局部结构保持接近原始子图。 在多个 TAG 数据集和代表性 GFMs 上的大量实验表明了 STAG 的有效性和隐蔽性。 我们的代码可在 https://github.com/ventr1c/STAG 获取。

### 37. Rethinking Demonstration Unlearning in Imitation Learning for Robotics
...
### 38. Generating Multi-view Adversarial Examples for Visual Geometry Grounded Transformer
...
### 39. MotionPhys: Detecting AI-Generated Videos via Physical Consistency of Optical-Flow Trajectories
...
### 40. LoRC: Detecting AI-Generated Images via Low-Rank Collapse in Semantic Residuals
...
### 41. Breaking High Confidence: Practical Face Impersonation under High-Security Thresholds

...
- 作者及单位：
  - Marek Mateusz Kowalski — 单位未核实
  - Joshua Fonseca Rivera — 单位未核实
  - Uzay Macar — 单位未核实
...
### 65. Beyond Verdicts: A Graph-Based Analysis of Human and LLM Reasoning in Scientific Fact-Checking

**内容概述：** 针对科学 fact-checker 只看 verdict 或文本解释、无法判断模型是否沿可靠证据路径推理，论文用 typed reasoning graph 对齐人类与 LLM 的 claim、研究背景、前提和谬误子图；84 条生物医学误导主张上，不同模型分别在判断失败率、人类路径对齐和有效替代推理方面表现出明显分工。

- 收录理由：论文针对歪曲真实科研论文的错误信息，审计事实核查模型的证据依据和推理路径，而不仅检查最终判断。
- 论文/arXiv：[Beyond Verdicts: A Graph-Based Analysis of Human and LLM Reasoning in Scientific Fact-Checking](https://arxiv.org/abs/2608.23047)
- arXiv ID：`2608.23047`
- 代码：暂未找到公开代码
- arXiv 分类：`cs.CL`、`cs.AI`
- 提交日期：2026-08-24
...
### 67. FIDES: A Concordance Protocol for LLM-Generated Trading Strategies
...
### 69. Adversarial Entropy Inflation Against Gumbel-Based Inference Verification
